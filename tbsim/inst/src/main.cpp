...
int main(int argc, char* argv[])
{
    ...
    bool configStatus(true), therapyStatus(true), adherenceStatus(true), MEMSStatus(true), drugStatus(true), immuneStatusParams(true);//, immuneStatusParams(true)
    ...
    if ((validFile)&&(configStatus)) {
        ...
        do {
            ...
            if ((PARA.batchMode==1) || (inputValue==0)) {
                ...
                immuneStatusParams=IMM.readImmuneParameters(PARA.disease, configFolder, PARA.immuneFile);
                IMM.setImmuneStatus(PARA.nPatients, PARA.immuneMean, PARA.immuneStdv);
                
                if (!immuneStatusParams){std::cout << "Note: Immune file status load fail"<<std::endl;}
