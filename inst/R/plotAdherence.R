...
plotAdherence <- function(type){
	...
	H1 <- hist(adhTot)	
	barplot(H1$counts/nPatients, main="Non-adherent days per patient", ylim=c(0,1.5*max(H1$counts/nPatients)),
	...
	H2 <- hist(adhCount)
	barplot(H2$counts/nPatients, main="Non-adherence events per patient", ylim=c(0,1.5*max(H2$counts/nPatients)),
