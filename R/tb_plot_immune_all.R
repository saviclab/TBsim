...
# Oct 6, 2014
#===========================================================================
tb_plot_immune_all <- function(info, immune) {
  ...
  with (immune, {
    ...
    pl1 <- tb_plot_immune(df1, c("time", "IL4", "IL10", "IL12L", "IFN"), mainTitle, subTitle, "pg/mL", drugStart)

    # Cytokines in Lymph Node (IL12LN)
    mainTitle <- "Cytokines in Lymph Node (IL12LN)"
    pl2 <- tb_plot_immune(df2, c("time", "IL12LN"), mainTitle, subTitle, "pg/mL", drugStart)

    # Dendritic Cells, (IDC and MDC)
    mainTitle <- "Dendritic Cells (IDC and MDC)"
    pl3 <- tb_plot_immune(df3, c("time", "IDC", "MDC"), mainTitle, subTitle, "Cells/mL", drugStart)

    # T cells (T1, T2)
    mainTitle <- "T cells in Lung (T1, T2)"
    pl4 <- tb_plot_immune(df4, c("time", "T1", "T2"), mainTitle, subTitle, "Cells/mL", drugStart)

    # T helper cells (Tp, TpLN)
    mainTitle <- "T cells in Lung (Tp, TpLN)"
    pl5 <- tb_plot_immune(df5, c("time", "Tp", "TpLN"), mainTitle, subTitle, "Cells/mL", drugStart)

    # Naive T cells (TLN)
    mainTitle <- "Naive T cells in Lymph (TLN)"
    pl6 <- tb_plot_immune(df6, c("time", "TLN"), mainTitle, subTitle, "Cells/mL", drugStart)
